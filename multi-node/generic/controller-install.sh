...
EOF
    ...
    local TEMPLATE=/etc/systemd/system/docker.service.d/50-proxy.conf
    [ -z $HTTP_PROXY ] && [ -f $TEMPLATE ] || {
        echo "TEMPLATE: $TEMPLATE"
        mkdir -p $(dirname $TEMPLATE)
        cat << EOF > $TEMPLATE
[Service]
...
EOF
    ...
    local TEMPLATE=/etc/systemd/system/early-docker.service.d/50-proxy.conf
    [ -z $HTTP_PROXY ] && [ -f $TEMPLATE ] || {
        echo "TEMPLATE: $TEMPLATE"
        mkdir -p $(dirname $TEMPLATE)
        cat << EOF > $TEMPLATE
